...
export default function AddReminder() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ ...payload, action: 'addreminder' }),
      });
  ...
  const handleCalendarDateChange = (day: { dateString: string }) => {
    ...
    setDatetime((prevDatetime) => {
      const updated = new Date(prevDatetime);
  ...
  const handleTimePickerChange = (event: any, selected?: Date) => {
    if (selected) {
      ...
      setSelectedTime(`${String(selectedHour).padStart(2, '0')}:${String(selectedMinute).padStart(2, '0')}`);
      setDatetime((prevDatetime) => {
        const updated = new Date(prevDatetime);
  ...
  return (
    <View style={styles.container}>
      ...
      {showTimePicker && (
        <Card style={styles.calendarCard1}>
    ...
    <TouchableOpacity onPress={handleSubmit} style={styles.addButton}>
      <Text style={styles.addButtonText}>Reminder нэмэх</Text>
    </TouchableOpacity>
